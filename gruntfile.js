...
const env = {
  ...
  REPORTSDIR:   path.join( `${ strings.DIST  }`, `${ strings.REPORTS }` ),
  SRCDIR:       `${ strings.SRC   }`,
  STRINGS:      strings,
  TEMPLATEDIR:  path.join( `${ strings.SRC   }`, `${ strings.TEST }`, `${ strings.TEMPLATE }` ),
  TESTDIR:      path.join( `${ strings.SRC   }`, `${ strings.TEST }` ),
...
module.exports = function( grunt ) {
  ...
  grunt.registerTask( strings.ALL,     [ strings.TEST,        strings.DOCS,
                                         strings.BUILD,       strings.DIST,
                                         strings.DEPLOY ]);
  ...
  // run test
  grunt.registerTask( strings.TEST,    [ strings.CHKOUTDATED,       strings.ESLINT,
                                         strings.CLEAN,             strings.MKDIR,
